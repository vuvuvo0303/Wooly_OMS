...
export const getCategories = async () => {
  try {
    ...
    const filteredData = data.filter((category: any) => !category.isDeleted);
...
export const createCategory = async (formData: { name: string }) => {
  try {
    const { data } = await axiosClient.post(`/category/add-category?name=${encodeURIComponent(formData.name)}`);
...
export const deleteCategory = async (categoryId: number) => {
  ...
  try {
    const response = await axiosClient.delete(`/category/delete-category/${categoryId}`);
...
export const updateCategory = async (categoryID: number, formData: { name: string }) => {
  try {
    const { data } = await axiosClient.put(
      `/category/update-category/${categoryID}?name=${encodeURIComponent(formData.name)}`
    );
